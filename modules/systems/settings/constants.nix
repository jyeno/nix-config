        ...
        keyboard = {
          binds =
            ...
            [
              ...
              ] "${wpctl} set-volume @DEFAULT_AUDIO_SINK@ 0.1+ -l 1.0" useWhenLocked)
              (mkBindEx "" [
                "XF86AudioLowerVolume"
              ] "${wpctl} set-volume @DEFAULT_AUDIO_SINK@ 0.1-" useWhenLocked)
              (mkBindEx "" [ "XF86AudioMute" ] "${wpctl} set-mute @DEFAULT_AUDIO_SINK@ toggle" useWhenLocked)
              (mkBindEx "SHIFT" [
                "XF86AudioMute"
              ] "${wpctl} set-mute @DEFAULT_AUDIO_SOURCE@ toggle" useWhenLocked)
