      ...
      bind: let
        ...
        extraModsGen = extras:
          builtins.concatStringsSep " " (builtins.map (
            n:
              if n == "locked"
              then "allow-when-locked=${lib.boolToString extras.locked}"
              else if n == "description"
              then "hotkey-overlay-title=\"${extras.description}\""
              else ""
          ) (builtins.attrNames extras));
      in "${prefix}${builtins.concatStringsSep "+" bind.keys} ${extraModsGen bind.extras} { spawn-sh \"${bind.cmd}\"; }"
    )
    desktop.keyboard.binds;
in {
  options.local.home.desktop.niri = {
  ...
  config = lib.mkIf cfg.enable {
    ...
    xdg.configFile."niri/config.kdl".text = ''
        ...
        input {
          ...
          keyboard {
            xkb {
              layout "${desktop.keyboard.xkb.layout}"
              variant "${desktop.keyboard.xkb.variant}"
              options "${desktop.keyboard.xkb.options}"
            }
            repeat-delay 250
            repeat-rate 40
            track-layout "global"
