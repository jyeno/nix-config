{
  flake.modules.homeManager.cli-neomutt = {
    pkgs,
    config,
    ...
  }: let
    # Collect all addresses and aliases
    addresses = pkgs.lib.flatten (
      pkgs.lib.mapAttrsToList (_: v: [v.address] ++ v.aliases) config.accounts.email.accounts
    );
  in {
    xdg = {
      desktopEntries = {
    ...
    programs.neomutt = {
      ...
      extraConfig =
        ''
          alternates "${pkgs.lib.concatStringsSep "|" addresses}"
        ''
        + ''
